...
# SETTING
dir_path = "C:/workspace/SocialMovementSentiment/Benchmark/MinneapolisPoliceStopDataset/"
...
# FIXED DATA ISSUE
df['responseDate'] = df['responseDate'].astype('datetime64[ns]')
...
df = df.loc[(df['responseDate'] >= start_date) & (df['responseDate'] <= end_date)]
df['responseDate'] = df['responseDate'].apply(lambda x: datetime.strptime(str(x), DateHelper.standard_date_format).date())
df_group = df[["responseDate", "masterIncidentNumber", "reason"]].groupby(["responseDate", "reason"], as_index=False).count()
df_group["masterIncidentNumber"] = df_group["masterIncidentNumber"]*15
...
fig = sns.lineplot(x="responseDate", y="masterIncidentNumber", hue="reason", palette="muted", data=df_group)
...
plt.title("Total #BLM Tweets v.s. Police Stops")
