...
# SETTING
dir_path = "C:/workspace/SocialMovementSentiment/SecondDataset/MinneapolisCovidCases/"
...
def generate_plot(df_s, df_p, start_date, end_date, x_attr, y_attr, y_factor):
    ...
    df_s[y_attr] = df_s[y_attr]*y_factor
    ...
    fig = sns.lineplot(x=x_attr, y=y_attr, palette="muted", data=df_s)
    ...
    plt.title("Total #BLM Tweets v.s. Minneapolis Covid Cases", fontsize='25')
    plt.savefig("{}img/lineplot_comparison.png".format(dir_path))
    plt.close()


generate_plot(df, primary_df_group, start_date, end_date, "submission_date", "COVID-19 Deaths", 10)
