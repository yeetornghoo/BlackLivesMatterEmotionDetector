...
# SETTING
dir_path = "C:/workspace/SocialMovementSentiment/SecondDataset/MinneapolisPoliceStop/"
...
def generate_plot(df_s, df_p, start_date, end_date, x_attr, y_attr, h_attr, y_factor, sub_report_name):
    ...
    plt.title("Total #BLM Tweets v.s. Minneapolis Police Stops ({})".format(sub_report_name.replace("_", " ")).upper(), fontsize='25')
    plt.savefig("{}img/lineplot_comparison_{}.png".format(dir_path, sub_report_name))
    plt.close()


generate_plot(df, primary_df_group, start_date, end_date, "responseDate", "masterIncidentNumber", "reason", 10, "by_reason")
generate_plot(df, primary_df_group, start_date, end_date, "responseDate", "masterIncidentNumber", "race", 10, "by_race")
generate_plot(df, primary_df_group, start_date, end_date, "responseDate", "masterIncidentNumber", "gender", 10, "by_gender")
generate_plot(df, primary_df_group, start_date, end_date, "responseDate", "masterIncidentNumber", "problem", 10, "by_problem")
generate_plot(df, primary_df_group, start_date, end_date, "responseDate", "masterIncidentNumber", "callDisposition", 10, "by_callDisposition")
generate_plot(df, primary_df_group, start_date, end_date, "responseDate", "masterIncidentNumber", "personSearch", 10, "by_personSearch")
generate_plot(df, primary_df_group, start_date, end_date, "responseDate", "masterIncidentNumber", "vehicleSearch", 10, "by_vehicleSearch")